...
const UpcomingSimilar = () => {
  const [similarShowList, setSimilarShowList] = useState([]);
  const movieID = localStorage.getItem("clickedUpcomingID");
      ...
      <div>
        ...
        <ul className="similar_horizontal-scroll">
          {similarShowList.map((item) => (
            <li>
              <Link
                to={`/upcomingsimilar/${item.id}`}
                onClick={() => {
                  localStorage.setItem("upcomingsimilar", item.id);
                }}
              >
                <img
...
export default UpcomingSimilar;
